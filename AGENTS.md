...
sets `build.args` to enable Codex (the Dockerfile ARGs default `false`);
...
| `skills/` | Curated skills (portable `SKILL.md` format), optionally copied with `--skills` into each selected CLI's skills dir — `.claude/skills/` for Claude, `.agents/skills/` for Codex, `.opencode/skills/` for OpenCode, `.kiro/skills/` for Kiro. Skills tagged `metadata: author: mattpocock` are refreshed by `vendor-matt-pocock-skills.sh`; complete standalone skill directories tagged `metadata.source: specterops` are refreshed from SpecterOps by `vendor-specterops-skills.sh`. Both helpers are maintainer-only and excluded from generated projects, while every nested support file belonging to a skill is copied. |
...
- **docker-compose.yml** — rendered from `templates/docker-compose.yml` (the
  clean, no-args copy) with managed selected-CLI environment/volume sections.
  On rerun, only those marked sections are replaced, preserving unrelated user
  Compose edits. The repo's own `.devcontainer/docker-compose.yml` is *not*
  shipped, so its maintainer `build.args` never leak into projects.
- **Native AI CLI configuration** — selected CLIs receive active `/app`-based
  state and update-disable settings in generated Compose. Native config files
  are merged without replacing unrelated keys. `.env.example` is always copied;
...
(respects `--force` / interactive overwrite prompt / no-tty skip). A generated
Compose file with managed CLI markers is updated on rerun only inside those
markers, like the merge exceptions below. Prompts read from `/dev/tty` so they
...
2. Register the token in `install.sh` — a **tool** in `VALID_TOOLS` + `tool_arg()`,
   or an **AI CLI** in `VALID_CLIS` + `cli_arg()` (and, for a CLI,
   `cli_skills_dir()` plus its native state/config mappings).
...
### Change AI CLI configuration
Edit the CLI state/config assembly in `install.sh`, the commented provider
mappings in `templates/docker-compose.yml`, and `.env.example` together. Keep
state paths absolute under `/app`, preserve native config keys on reruns, and do
not pass `.env` wholesale into the container.
